...
// ==============================================================
`timescale 1ns/1ps
module pl_kernel_control_s_axi
#(parameter
    ...
    output wire [63:0]                   xUpdate,
    output wire [63:0]                   cost,
...
localparam
    ...
    ADDR_IER            = 7'h08,
    ADDR_ISR            = 7'h0c,
    ADDR_X_IN_DATA_0    = 7'h10,
    ADDR_X_IN_DATA_1    = 7'h14,
    ADDR_X_IN_CTRL      = 7'h18,
    ADDR_XUPDATE_DATA_0 = 7'h1c,
    ADDR_XUPDATE_DATA_1 = 7'h20,
    ADDR_XUPDATE_CTRL   = 7'h24,
    ADDR_COST_DATA_0    = 7'h28,
    ADDR_COST_DATA_1    = 7'h2c,
    ADDR_COST_CTRL      = 7'h30,
    ADDR_ATY_DATA_0     = 7'h34,
    ADDR_ATY_DATA_1     = 7'h38,
    ADDR_ATY_CTRL       = 7'h3c,
    ADDR_ALPHA_DATA_0   = 7'h40,
    ADDR_ALPHA_DATA_1   = 7'h44,
    ADDR_ALPHA_CTRL     = 7'h48,
    ADDR_LB_DATA_0      = 7'h4c,
    ADDR_LB_DATA_1      = 7'h50,
    ADDR_LB_CTRL        = 7'h54,
    ADDR_UB_DATA_0      = 7'h58,
    ADDR_UB_DATA_1      = 7'h5c,
    ADDR_UB_CTRL        = 7'h60,
    ADDR_LEN_DATA_0     = 7'h64,
    ADDR_LEN_CTRL       = 7'h68,
    WRIDLE              = 2'd0,
    WRDATA              = 2'd1,
    WRRESP              = 2'd2,
...
//------------------------Local signal-------------------
    ...
    reg  [63:0]                   int_xUpdate = 'b0;
    reg  [63:0]                   int_cost = 'b0;
    reg  [63:0]                   int_aty = 'b0;
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_XUPDATE_DATA_0: begin
                    rdata <= int_xUpdate[31:0];
                end
                ADDR_XUPDATE_DATA_1: begin
                    rdata <= int_xUpdate[63:32];
                end
                ADDR_COST_DATA_0: begin
                    rdata <= int_cost[31:0];
                end
                ADDR_COST_DATA_1: begin
                    rdata <= int_cost[63:32];
                end
                ...
                ADDR_LB_DATA_0: begin
                    rdata <= int_lb[31:0];
                end
                ADDR_LB_DATA_1: begin
                    rdata <= int_lb[63:32];
                end
                ADDR_UB_DATA_0: begin
                    rdata <= int_ub[31:0];
                end
                ADDR_UB_DATA_1: begin
                    rdata <= int_ub[63:32];
                end
...
assign xUpdate           = int_xUpdate;
assign cost              = int_cost;
assign aty               = int_aty;
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_XUPDATE_DATA_0)
            int_xUpdate[31:0] <= (WDATA[31:0] & wmask) | (int_xUpdate[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_XUPDATE_DATA_1)
            int_xUpdate[63:32] <= (WDATA[31:0] & wmask) | (int_xUpdate[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COST_DATA_0)
            int_cost[31:0] <= (WDATA[31:0] & wmask) | (int_cost[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_COST_DATA_1)
            int_cost[63:32] <= (WDATA[31:0] & wmask) | (int_cost[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_LB_DATA_0)
            int_lb[31:0] <= (WDATA[31:0] & wmask) | (int_lb[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_LB_DATA_1)
            int_lb[63:32] <= (WDATA[31:0] & wmask) | (int_lb[63:32] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_UB_DATA_0)
            int_ub[31:0] <= (WDATA[31:0] & wmask) | (int_ub[31:0] & ~wmask);
    end
end
...
always @(posedge ACLK) begin
    if (ARESET)
        ...
        if (w_hs && waddr == ADDR_UB_DATA_1)
            int_ub[63:32] <= (WDATA[31:0] & wmask) | (int_ub[63:32] & ~wmask);
    end
end
